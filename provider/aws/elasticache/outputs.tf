...
output "redis_reader_endpoint" {
  value       = var.cluster_enabled ? "" : (var.cluster_replication_enabled ? join("", aws_elasticache_replication_group.cluster.*.configuration_endpoint_address) : join("", aws_elasticache_replication_group.default.*.reader_endpoint_address))
  description = "Redis endpoint address."
}
